/* Hypermedia workflow. 
 ** Implementation of Finite State Machine for a 'choice' task. This task waits for all inputs
 ** and produces signals to only selected outputs. 
 ** Inputs:
 ** - a number of data ports x1 .. xn
 ** Outputs:
 ** - a number of data ports y1 .. ym
 ** Function:
 ** - The function determines which of the m outputs should be passed to their sinks. To this
 **   end, the function should insert a pair << "condition": true" >> into the json objects of 
 **   appropriate output data elements.
 ** 
    ...
    this.running_onEnter = function(session, state, transition, msg) {
        ...
        (function(task) {
            task.wflib.setTaskState(task.wfId, task.id, { "status": "running" }, function(err, rep) {
                ...
                task.wflib.invokeTaskFunction(task.wfId, task.id, task.ins, task.outs, emul, function(err, rep) {
                    if (err) {
                        throw(err);
                        ...
                        var outs = [];
                        rep.forEach(function(out) {
                            if (out.condition == "true") {
                                // attribute 'condition' is only relevant here for making the decision on which signals to emit.
                                // it's safe (and cleaner) to remove it before passing the signal on
