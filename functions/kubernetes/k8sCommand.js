...
async function k8sCommandGroup(bufferItems) {
  ...
  let customParamsPartition = partition
    ? context.appConfig["jobvars" + partition]
    : null;
  if (customParamsPartition) {
  ...
  try {
    ...
    jobExitCodes = await synchronizeJobs(
      jobArr,
      taskIdArr,
      contextArr,
      customParams,
  ...
  console.log(
    "Ending k8sCommandGroup function, time:",
    endTime,
    "exit codes:",
    jobExitCodes
  );
  ...
  for (var i = 0; i < jobExitCodes.length; i++) {
    ...
    if (
      jobExitCode != 0 &&
      process.env.HF_VAR_STOP_WORKFLOW_WHEN_JOB_FAILED == "1"
    ) {
